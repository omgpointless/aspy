...
impl FocusablePanel {
    /// Cycle to next panel (Tab behavior)
...
mod tests {
    ...
    fn test_focus_cycling() {
        let focus = FocusablePanel::Events;
        assert_eq!(focus.next(), FocusablePanel::Thinking);
        assert_eq!(focus.next().next(), FocusablePanel::Logs);
        assert_eq!(focus.next().next().next(), FocusablePanel::Events);
    }
